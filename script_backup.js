...
function fill(context, x, y, colour) {
    ...
    let target_colour = getPixelColour(context, x, y);

    //check if a pixel has already been checked
    let visited = [];
    ...
    //create a stack to hold the coordinates of pixels to be filled
    let pixels = [{ x: x, y: y }];
    let count = 0;
    ...
    while (pixels.length > 0) {
        ...
        if (
            ...
            pixels.push({ x: current_x + 5, y: current_y });
            pixels.push({ x: current_x - 5, y: current_y });
            pixels.push({ x: current_x, y: current_y + 5 });
            pixels.push({ x: current_x, y: current_y - 5 });
        }
        count++;
        if (count > 1000) {
